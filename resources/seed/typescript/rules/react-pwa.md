...
- Deep relative imports crossing layers (`../../../`) are a smell — use the project's configured path aliases (e.g. `@shared/*`).

## PWA / Service Worker (when the app ships one)
...
## i18n (when the app is localized)

- No user-facing string hardcoded in the UI — every label goes through a translation key.
- Always use i18next `count` for plurals (rules differ per language) — never concatenate strings.
- One single map `i18n locale → date-fns locale`; date/number formatting respects the active locale.
- Key parity across every shipped locale is checked in CI — a missing key is a BLOCKER.
- Test layout with the most verbose shipped locale (German runs ~30% longer than English) besides the default one.
