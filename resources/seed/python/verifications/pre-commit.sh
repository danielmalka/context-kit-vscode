#!/bin/sh
# Pre-commit quality gate — Python
# Runs secrets scan, linting, format check, type check, tests, dependency audit, and SAST.
# Exits non-zero if any check fails, blocking the commit.

set -e

echo "=== Quality Gate: Pre-commit (Python) ==="

echo ""
echo "[1/7] secrets scan..."
...
echo ""
echo "[6/7] pip-audit (dependency vulnerabilities)..."
uv run pip-audit
echo "  OK"

echo ""
echo "[7/7] bandit (SAST)..."
uv run bandit -r . -ll --exclude ".venv,tests"
echo "  OK"
